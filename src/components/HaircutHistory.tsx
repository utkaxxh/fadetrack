  ...
  if (filteredHaircuts.length === 0) {
    return (
      <div className="text-center py-12">
        <div className="text-gray-400 dark:text-gray-500 text-lg mb-2">No haircuts logged yet</div>
        <div className="text-gray-500 dark:text-gray-400 text-sm">Start by logging your first haircut!</div>
      </div>
    );
  }
    ...
    <div className="max-w-3xl mx-auto">
      <div className="space-y-4">
        {filteredHaircuts.map((cut, i) => (
          <div key={i} className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6 hover:shadow-md transition-shadow duration-200">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  <span className="text-lg font-semibold text-gray-900 dark:text-gray-100">{cut.barber}</span>
              ...
              <div className="text-right">
                <div className="text-2xl font-bold text-green-600 dark:text-green-400">${cut.cost}</div>
              </div>
            </div>
          </div>
        ))}
